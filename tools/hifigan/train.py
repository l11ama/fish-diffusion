...
from fish_diffusion.modules.vocoders.hifigan.generator import HiFiGANGenerator
...
class RefineGAN(pl.LightningModule):
    def __init__(self, config):
        ...
        self.generator = HiFiGANGenerator(**config.model.generator)
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_g_hat = self.generator(gt_mels)
        ...
        if batch_idx % self.config.grad_acc_steps == 0:
            optim_d.step()
            optim_d.zero_grad()
    ...
    def get_mels(self, audios, transform=None):
        ...
        x = transform(audios.squeeze(1).float())
